...
def _run_backtest(symbol: str, timeframe: str,
                  config: dict, capital: float,
                  start: str | None = None, end: str | None = None) -> dict | None:
    from fibot.analysis.backtester import load_ohlcv, run_backtest
    if end is None:
    ...
    if data is None or data.empty or len(data) < 50:
        return None
    result = run_backtest(data, config, capital, symbol, timeframe)
    if result is None:
        return None
    return {
    ...
    for c in selected:
        ...
        entry = {
            **base,
            ...
            'leverage':           c.get('leverage', base.get('leverage', 2)),
            'margin_mode':        base.get('margin_mode', 'isolated'),
            'risk_per_entry_pct': c.get('risk_per_entry_pct', base.get('risk_per_entry_pct', 0.5)),
            'active':             True,
        }
...
def main() -> int:
    ...
    for path in tqdm(config_files, desc='Backteste Configs'):
        try:
            symbol, timeframe, cfg = _parse_config(path)
            if not symbol or not timeframe:
                continue
            result = _run_backtest(symbol, timeframe, cfg, capital, start=start_date, end=end_date)
            if not result:
                continue
            risk = cfg.get('risk', {})
            candidates.append({
                ...
                'risk_per_entry_pct': risk.get('risk_per_entry_pct', 0.5),
            })

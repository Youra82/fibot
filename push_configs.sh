...
CONFIG_COUNT=$(ls "$CONFIGS_DIR"/config_*_fib.json 2>/dev/null | wc -l)
...
for f in "$CONFIGS_DIR"/config_*_fib.json; do
    echo "  - $(basename "$f")"
done
echo ""
...
git commit -m "Update: Fibonacci-Konfigurationen aktualisiert ($TIMESTAMP)"
